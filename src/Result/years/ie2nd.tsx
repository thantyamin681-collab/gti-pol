...
const studentResults: StudentResult[] = [
  { no: 1, rollNo: '2GIE-1', name: 'မမေချမ်းသာ', marks: ['A-', 'A+', 'A-', 'A', 'A+', 'B+', 'A', 'A-'] },
  { no: 2, rollNo: '2GIE-2', name: 'မချစ်မြတ်မြတ်ဇော်', marks: ['B+', 'A+', 'B+', 'A-', 'A+', 'B', 'A', 'B+'] },
  { no: 3, rollNo: '2GIE-4', name: 'မောင်ကိုကိုပိုင်', marks: ['D', 'B-', 'Inc', 'D', 'B+', 'Inc', 'B+', 'Inc'] }
];

const calculateResult = (marks: string[]): string => {
  if (marks.includes('Inc') || marks.includes('Fail')) {
    return 'Fail';
  }
  return 'Pass';
};

export function IeYear2({ onBack }: IeYear2Props) {
    ...
    <div className="results-wrapper">
      <div className="results-container" style={{ maxWidth: '100%', padding: '2rem 3rem' }}>
        ...
        <div className="results-header">
          <h2>A.G.T.I. (Industrial Engineering) - Second Year Results</h2>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center', minWidth: '1100px' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #e2e8f0', color: '#64748b', fontSize: '0.8rem' }}>
                ...
                <th style={{ padding: '10px' }}>Computer Aided Design</th>
                <th style={{ padding: '10px' }}>Control Engineering (Pneumatics and Electro-Pneumatics)</th>
                <th style={{ padding: '10px' }}>Industrial Wiring Installation and Motor Control I</th>
                <th style={{ padding: '10px' }}>Digital Electronics I</th>
