...
/**
 * SchoolInfo component (merged from second file)
 * - Props:
 *    selectedDeptProp?: number | null         // optional initial / controlled selected dept from parent
 *    onSelectedDeptChange?: (n: number|null)  // optional callback to notify parent of selection changes
 *    onNavigate?: (target?: any)              // optional navigation helper from parent
 */
export function SchoolInfo({
  ...
  selectedDeptProp?: number | null;
  onSelectedDeptChange?: (n: number | null) => void;
  onNavigate?: (target: any) => void;
}) {
  // local selected dept but keep it in sync with parent via props/callbacks
  const [selectedDeptLocal, setSelectedDeptLocal] = useState<number | null>(selectedDeptProp ?? null);

  useEffect(() => {
    // when parent prop changes, sync into local state
    if (selectedDeptProp !== undefined && selectedDeptProp !== selectedDeptLocal) {
      setSelectedDeptLocal(selectedDeptProp);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedDeptProp]);

  // wrapper to set local and inform parent
  function setSelectedDept(n: number | null) {
    setSelectedDeptLocal(n);
    if (onSelectedDeptChange) onSelectedDeptChange(n);
  }
      ...
      <div className="min-h-screen bg-white p-6">
        ...
        <div className="max-w-3xl mx-auto text-center">
          {coursesView.page === 'years' && (
            <>
              ...
              <div className="flex gap-6 justify-center">
                {coursesData[selectedDeptLocal]?.map((y) => (
                  <button
                    key={y.yearNumber}
          ...
          {coursesView.page === 'semesters' && (
            <>
              <h1 className="text-2xl font-extrabold text-blue-900 mb-4">
                {coursesData[selectedDeptLocal]?.find((d) => d.yearNumber === coursesView.yearNumber)?.yearLabel}
              </h1>
              <p className="mb-6 text-gray-700">Choose a Semester</p>

              <div className="flex gap-6 justify-center">
                {coursesData[selectedDeptLocal]?.find((d) => d.yearNumber === coursesView.yearNumber)
                  ?.semesters.map((s) => (
                    <button
                      key={s.id}
          ...
          {coursesView.page === 'semesterDetail' && (
            <>
              {(() => {
                const year = coursesData[selectedDeptLocal]?.find((d) => d.yearNumber === coursesView.yearNumber);
  ...
  if (selectedDeptLocal !== null) {
    const dept = departments[selectedDeptLocal];
      ...
      <div className="min-h-screen bg-white p-6">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-6">
            ...
            {selectedDeptLocal !== null && selectedDeptLocal < 7 && (
              <div>
                <button
...
/**
 * Top-level App component (merged from first file)
 * - handleNavigate now accepts either a string or an object payload { view, deptIndex? }.
 * - Maintains schoolInfoDept in parent state to be able to open SchoolInfo with specific department.
 * - Stores deptIndex in history.state for popstate handling (back/forward).
 */
function App() {
  // 'home', 'login', 'admin', 'news', 'schoolinfo' navigation states
  const [view, setView] = useState<'home' | 'login' | 'admin' | 'news' | 'schoolinfo'>('home');

  // keep a parent selected dept to allow Navbar -> SchoolInfo deep-linking
  const [schoolInfoDept, setSchoolInfoDept] = useState<number | null>(null);

  useEffect(() => {
    const handlePopState = (ev?: PopStateEvent) => {
      const p = window.location.pathname;
      const state = window.history.state as { deptIndex?: number } | null;

      if (p === '/latest-news') {
        setView('news');
      } else if (p === '/school-info') {
        setView('schoolinfo');
        if (state && typeof state.deptIndex === 'number') {
          setSchoolInfoDept(state.deptIndex);
        } else {
          // if no dept in state, keep existing parent-dept (or null)
          // setSchoolInfoDept(null);
        }
      } else {
        setView('home');
      }
   ...
   * Accept either:
   * - handleNavigate('news')
   * - handleNavigate({ view: 'schoolinfo', deptIndex: 2 })
   */
  const handleNavigate = (target: any) => {
    ...
    // object form: { view, deptIndex? }
    if (typeof target === 'object' && target !== null) {
      const viewName = target.view as string | undefined;
      const deptIndex = typeof target.deptIndex === 'number' ? target.deptIndex : undefined;

      if (viewName === 'schoolinfo') {
        setView('schoolinfo');
        setSchoolInfoDept(typeof deptIndex === 'number' ? deptIndex : null);
        // include deptIndex in history state so popstate can restore
        window.history.pushState({ deptIndex }, '', '/school-info');
  ...
  return (
    <div>
      ...
      {view === 'home' && (
        <div>
          {/* GTIHomePage should call onNavigate with either a string or an object payload.
              Example usages in GTIHomePage:
                onNavigate('schoolinfo')
                onNavigate({ view: 'schoolinfo', deptIndex: 2 })
          */}
          <GTIHomePage onNavigate={(target: any) => handleNavigate(target)} />
      ...
      {/* 2. Login View */}
      {view === 'login' && <GTILogin onLoginSuccess={() => handleNavigate('admin')} onBackToHome={() => handleNavigate('home')} />}

      {/* 3. Admin Dashboard View */}
      {view === 'admin' && <GTIAdminDashboard onBackToHome={() => handleNavigate('home')} />}

      {/* 4. Latest News View */}
      {view === 'news' && <SchoolNews onBack={() => handleNavigate('home')} />}

      {/* 5. School Info View */}
      {view === 'schoolinfo' && (
        <SchoolInfo
          // pass the parent-controlled dept and updater so we can deep-link from the navbar
          selectedDeptProp={schoolInfoDept}
          onSelectedDeptChange={(n) => setSchoolInfoDept(n)}
          // also hand down a navigation helper if SchoolInfo wants to navigate elsewhere
